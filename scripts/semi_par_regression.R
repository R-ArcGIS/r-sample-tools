...
tool_exec <- function(in_params, out_params)
{
  ...
  all_params = paste0(dep_variable, " ~ ",  fxy, "+", nonlin_params)
...
#print(all_params)
  ...
  pred_df['prediction_se'] = pred$se
  pred_df['lower_sign'] = 1 / (1+exp(-pred$fit+1.96*pred$se))
  pred_df['upper_sign'] = 1/(1+exp(-pred$fit-1.96*pred$se))
